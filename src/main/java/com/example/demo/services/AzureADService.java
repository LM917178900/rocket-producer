...
public class AzureADService {
	...
	private static String getAccessTokenUsingServicePrincipal(String clientId, String tenantId, String appSecret) throws MalformedURLException, InterruptedException, ExecutionException {
		...
		ConfidentialClientApplication.Builder authority = builder.authority(Config.authorityUrl + tenantId);
		ConfidentialClientApplication app = authority.build();


		Set<String> singleton = Collections.singleton(Config.scopeUrl);
		ClientCredentialParameters.ClientCredentialParametersBuilder parametersBuilder = ClientCredentialParameters.builder(singleton);
		ClientCredentialParameters clientCreds = parametersBuilder .build();


		CompletableFuture<IAuthenticationResult> iAuthenticationResultCompletableFuture = app.acquireToken(clientCreds);
		// Acquire new AAD token
		IAuthenticationResult result = iAuthenticationResultCompletableFuture.get();
